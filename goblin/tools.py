...
import sys
from goblin._compat import reraise, string_types, PY2
from goblin import connection
from goblin.exceptions import GoblinBlueprintsWrapperException
...
class BlueprintsWrapper(SessionPoolManager):
    ...
    :return: the used RexPro connection pool (with default session)
    :raises GoblinBlueprintsWrapperException: if no class name is provided
    """
    ...
    def __init__(self, class_name=None, setup=None, *args, **kwargs):
        ...
        if not class_name: # pragma: no cover
            raise GoblinBlueprintsWrapperException("A wrapper class name is required.")
...
class PartitionGraph(BlueprintsWrapper):
    ...
    :return: the used RexPro connection pool (with default session)
    :raises GoblinBlueprintsWrapperException: if no write partition is provided
    """

    def __init__(self, write=None, read=None, *args, **kwargs):
        if not write: # pragma: no cover
            raise GoblinBlueprintsWrapperException("A write partition is required.")
        read = read or []
