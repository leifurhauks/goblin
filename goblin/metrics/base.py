...
class BaseMetricsReporter(object):
    ...
    Customize this class for the specific metrics service, ie. Graphite,
    NewRelic, etc.
    """

    def __init__(self, registry=None, reportingInterval=5*60,
                 metric_prefix=None):
        """ Create a Metrics Reporter

        To override this method, please make sure to call the superclass
        *after* your methods. Ie.
        def __init__(self, mystuff, stuff2=None, *args, **kwargs):
            # do my stuff...
            super(MyClass, self).__init__(*args, **kwargs)

        :param registry: The pyformance registry
        :type registry: list[ MetricsRegistry | RegexRegistry ] |
            MetricsRegistry | RegexRegistry
        :param reportingInterval: The interval (number of seconds) on which to
            report the collected metrics.
        :type reportingInterval: float | long | int
        :param metric_prefix: The prefix on the collected metrics. The default
    ...
    def start_reporter(self, reportingInterval):
        ...
        Kicks off a loop that every reportingInterval runs the `send_metrics`
        method.
    ...
    def _get_metrics(self, timestamp=None, *args, **kwargs):
        """ Default pyformance implementation to collect all the metrics from
            the registries.

        Change only if you know what you are doing.

        :param timestamp: use this timestamp instead of the generated timestamp
    ...
    def get_metrics(self, timestamp=None, *args, **kwargs):
        ...
        Change this if you wan't customize for a particular metric collection
        system, ie. graphite, newrelic, etc.
...
__all__ = ['MetricsRegistry', 'RegexRegistry', 'Counter', 'Histogram', 'Meter',
           'Timer', 'GoblinMetricsException', 'BaseMetricsReporter',
