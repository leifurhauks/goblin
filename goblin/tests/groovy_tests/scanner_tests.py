...
from goblin.gremlin.groovy import (parse, GroovyImportParser,
                                   GroovyFunctionParser, GroovyImport,
                                   GroovyFunction, GroovyFileDef)


@attr('unit', 'groovy')
class GroovyScannerTest(BaseGoblinTestCase):
    """
    ...
    @classmethod
    def setUpClass(cls):
        cls.groovy_file = os.path.join(
            os.path.dirname(__file__), 'groovy_test_model.groovy')
    ...
    def test_parsing_example_import(self):
        ...
        self.assertListEqual(result.comment_list,
                             ['this', 'is', 'sample', 'text'])
        self.assertListEqual(result.import_list,
                             ['import com.wellaware.test;'])
