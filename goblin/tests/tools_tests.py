...
from goblin._compat import string_types, PY2
from nose.plugins.attrib import attr
from goblin.tests import BaseGoblinTestCase
from goblin.models import Vertex, Edge
from goblin import properties
from goblin.tools import (ImportStringError, import_string, cached_property,
                          LazyImportClass, _Missing, Factory)
import factory
from nose.tools import raises, nottest
from goblin import connection
from goblin.exceptions import GoblinQueryError
from goblin.tools import BlueprintsWrapper, PartitionGraph
# import eventlet


@attr('unit', 'tools')
class TestImportString(BaseGoblinTestCase):
    ...
    def test_imports_correctly(self):
        cp = import_string('goblin.tools:cached_property')
        self.assertIs(cp, cached_property)

    def test_import_string_error_class(self):
        ise = ImportStringError('goblin.tools.cached_property', 'Should work')
...
@attr('unit', 'tools')
class TestLazyImportString(BaseGoblinTestCase):

    def test_lazy_import(self):
        li = LazyImportClass('goblin.tools._Missing')
...
class TestModelFactory(BaseGoblinTestCase):
...
class TestBlueprintsWrapper(BaseGoblinTestCase):
    ...
    @raises(RuntimeError, GoblinQueryError)
    ...
    def test_partition_graph(self):
        ...
        with PartitionGraph(write='b', read=['a']):
            ...
            e1 = BlueprintsWrapperEdge.create(
                outV=v2, inV=v1, name="only in b")
            v3.name = "still in a"
            v3.save()
...
# class TestWrapperConcurrency(BaseGoblinTestCase):
#
#     @classmethod
#     def setUpClass(cls):
