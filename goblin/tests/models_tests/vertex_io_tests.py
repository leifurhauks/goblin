...
from goblin.tests.base import (
    BaseGoblinTestCase, TestVertexModel, TestEdgeModel, TestVertexModelDouble)
...
@attr('unit', 'vertex_io')
class TestVertexIO(BaseGoblinTestCase):
    ...
    def test_reload(self):
        """
        Tests that and instance's reload method does an inplace update of the
        instance
        """
        tm0 = yield TestVertexModel.create(test_val=8, name='123456789')
    ...
    @gen_test
    def test_all_empty_response(self):
        class SomeVertex(Vertex):
            pass
        resp = yield SomeVertex.all()
        while True:
            msg = yield resp.read()
            if msg is None:
                break
    ...
    @gen_test
    def test_all_ids_dont_exits(self):
        class SomeVertex(Vertex):
            pass
        resp = yield SomeVertex.all(['1'])
        with self.assertRaises(SomeVertex.DoesNotExist):
            msg = yield resp.read()

    @gen_test
    def test_all_method_with_invalid_id(self):
        v1 = yield TestVertexModel.create()
        v2 = yield TestVertexModel.create()
        try:
            from goblin.exceptions import GoblinQueryError
            with self.assertRaises(GoblinQueryError):
                stream = yield TestVertexModel.all([v1.id, v2.id, '1'])
                yield stream.read()
        finally:
            yield v1.delete()
            yield v2.delete()
    ...
    def test_find_by_value_method(self):
        ...
        try:
            ...
            resp = yield TestVertexModelDouble.find_by_value(
                'test_val', -100.0)
    ...
    def test_get_by_id(self):
        ...
        try:
            ...
            with self.assertRaises(TestEdgeModel.DoesNotExist):
                results = yield TestVertexModel.get('1')
...
@attr('unit', 'vertex_io')
class TestNestedDeserialization(BaseGoblinTestCase):
    """
    Tests that vertices are properly deserialized when nested in map and list
    data structures
    """
