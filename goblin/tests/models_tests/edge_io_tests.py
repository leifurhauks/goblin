...
from goblin.tests.base import (BaseGoblinTestCase, TestVertexModel,
                               TestEdgeModel, TestEdgeModel2,
                               TestEdgeModelDouble)
from goblin.exceptions import ValidationError, GoblinQueryError
from goblin.models import Edge


@attr('unit', 'edge_io')
class TestEdgeIO(BaseGoblinTestCase):
    ...
    def test_all_method_for_known_bad_input(self):
        ...
        with self.assertRaises(TestEdgeModel.DoesNotExist):
            stream = yield TestEdgeModel.all(['1'])
            results = yield stream.read()
    ...
    def test_reload_no_changes(self):
        # NOTE titan 0.4.2 and earlier changes made to an edge deletes and
        # then creates a new edge
        v1 = yield TestVertexModel.create(test_val=8, name='a')
        v2 = yield TestVertexModel.create(test_val=7, name='b')
        e1 = yield TestEdgeModel.create(v1, v2, test_val=3)
        try:
