...
from goblin.tests.base import (
    BaseGoblinTestCase, TestVertexModel, TestEdgeModel)
from goblin.exceptions import ModelException, GoblinException, ValidationError
...
class TestModelClassFunction(BaseGoblinTestCase):
    """
    ...
    def test_graph_property_attributes_handled_correctly(self):
        """
        Tests that graph property attributes are moved to a _properties dict
    ...
    def test_db_map(self):
        ...
        The db_map allows graph properties to be named something other than
        their variable name
        """
...
@attr('unit', 'class_construction')
class TestManualTableNaming(BaseGoblinTestCase):
...
@attr('unit', 'class_construction')
class TestAbstractElementAttribute(BaseGoblinTestCase):
